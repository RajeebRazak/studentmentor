...
app.get("/", function (req, res) {
  if (req.url == "/") {
    ...
    <div style="display:flex; justify-content:space-evenly; padding:20px;"> 
        <a href="/get-mentors" style="text-decoration:none; display:block;">All Mentors list</a>
        <a href="/get-students"  style="text-decoration:none; display:block;">All Students List</a>
    </div>
    
    `);
  } else {
    res.status(404).end(`
    <p style="font-family:'Open Sans', sans-serif">Please enter correct URL</p> 
    `);
  }
});
...
app.post("/students", async (req, res) => {
  try {
    const { studentName, studentMail, oldMentor, currentMentor } = req.body;
    const newStudent = {
